...
| Files, directories, paths, streams, JSON, and validation | covered | Module 6 [file lesson](../lessons/06_errors_files_json/03_files_and_defer/), [directory lesson](../lessons/06_errors_files_json/06_directory_operations/), exercise 6, Tasks storage/API contracts | Teaches file contents, directory trees, untrusted-boundary validation, ownership, and deterministic cleanup. | Advanced filesystems, symlink resolution policies, watchers, ACLs, and cross-filesystem moves are outside the beginner scope. |
